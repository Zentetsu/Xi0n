...
int moveDirectionRightMotor; // Move direction to the right motor
int moveDirectionLeftMotor;  // Move direction to the left motor
Serial myPort;               // xbee serial communication on the computer side
...
*******************************************************/
void draw() {
  if(myPort.available() > 0) readSerial();
  writeSerial();
  delay(100);
}
...
void readSerial() {
  ...
  if(frameStart == false) {
      ...
      if(frameSelectType == false) {
        ...
        } else System.out.println("The Frame Isn't Valid");
        frameFinish = true;
      }
    }
    ...
    if((frameFinish == true) || (increment == 3)) {
      ...
      else System.out.println("FRAME ERROR");
...
* FRAME TYPES:
*    => Changing operating mode:                          $0A!
*    => Changing the right engine duty cycle:             $1A!
*    => Changing the left engine duty cycle:              $2A!
*    => Changing the right engine movement direction:     $3A!
*    => Changing the left engine movement direction:      $4A!
****************************************************************/
void writeSerial() {
  /* OPERATING MODE FRAME */
  myPort.write("$0");
  if(mode == true) myPort.write("1");
  else myPort.write("0");
  myPort.write("!");
